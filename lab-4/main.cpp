...
static animationB *obj[50]; //limitation of balls
...
LRESULT CALLBACK WindowProcedure (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)
    {
        ...
        case WM_LBUTTONDOWN:
            ...
            obj[nrBall]=new Circle(newCenter, 4 + newCenter.x % 5 + newCenter.y % 5);
...
bool animationB::BallCollision(const RECT &rect) {
    if(center.x - 20 > rect.right - 50)
        {SpeedX=abs(SpeedX);}
    if(center.x + 20 < rect.left + 50)
        {SpeedX=-abs(SpeedX);}
